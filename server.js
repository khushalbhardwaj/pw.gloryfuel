...
// ---- PW (penpencil.co) token for direct video API access ----
...
function savePWToken(token, randomId, expiresIn) {
  ...
  writeJSON('pw_token.json', { token, randomId: pwRandomId, expiry: pwTokenExpiry });
}
loadPWToken();

// Batch slug map (batchId -> slug) for video-url-details parentId
let batchSlugMap = {};
(function buildBatchSlugMap() {
  try {
    const raw = readJSON('batches.json', {});
...
// Fetch from penpencil.co with PW token
async function pwCoFetch(method, path, opts = {}) {
  if (!pwToken) return { status: 401, body: { error: 'PW token not available' } };
  const headers = {
...
app.post('/api/admin/pw/send-otp', adminAuth, async (req, res) => {
...
app.post('/api/admin/pw/verify-otp', adminAuth, async (req, res) => {
...
app.post('/api/admin/pw/clear', adminAuth, (req, res) => {
  pwToken = null;
  pwRandomId = null;
  pwTokenExpiry = 0;
  writeJSON('pw_token.json', {});
  res.json({ ok: true });
});

// ---- DRM / Video Key endpoints (require PW token) ----
...
app.get('/api/study/otp', async (req, res) => {
  const { kid } = req.query;
  if (!kid) return res.status(400).json({ error: 'kid required' });
  if (!pwToken) return res.status(401).json({ error: 'PW token not available — login via admin panel' });
  try {
...
// Proxy MPD content from CDN (injects BaseURL pointing to our segment proxy)
app.get('/api/study/mpd', async (req, res) => {
  const { url } = req.query;
  if (!url) return res.status(400).json({ error: 'url required' });
  try {
...
// Segment proxy (attaches cached signed params to CloudFront URLs)
app.get('/api/study/proxy', async (req, res) => {
  const { url } = req.query;
  if (!url) return res.status(400).json({ error: 'url required' });
  let fetchUrl = url;
